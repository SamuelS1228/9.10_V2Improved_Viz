            ...
            def _build_lane_df(res, scn):
                ...
                for r in res["assigned"].itertuples():
                    ...
                    lanes.append(dict(
                        ...
                        rate=scn["rate_out"],
                        cost=cost,
                        center_idx=int(r.Warehouse),  # used for color-by-origin-warehouse
                    ))
                    ...
                    for j, ((wlon, wlat), dem) in enumerate(zip(res["centers"], demand_per_wh)):
                        ...
                        if dist > 1e-6 and dem > 0:
                            lanes.append(dict(
                                ...
                                center_idx=j,  # color by destination warehouse (origin tier1 is not a warehouse)
                            ))
                            ...
                            for (tx, ty), handled in zip(tier1_coords, t1_downstream_dem):
                                ...
                                if wt > 0:
                                    lanes.append(dict(
                                        ...
                                        center_idx=None,  # inbound to Tier-1; no warehouse origin
                                    ))
                    else:
                        for slon, slat, pct in scn["inbound_pts"]:
                            for j, ((wlon, wlat), wh_dem) in enumerate(zip(res["centers"], demand_per_wh)):
                                ...
                                if wt > 0:
                                    lanes.append(dict(
                                        ...
                                        rate=scn["in_rate"],
                                        cost=wt * dist * scn["in_rate"],
                                        center_idx=j,  # color by destination warehouse
                                    ))
