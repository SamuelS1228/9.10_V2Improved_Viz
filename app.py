    ...
    with tab:
        ...
        if st.session_state.get("run_target")==name:
            ...
            def _build_lane_df(res, scn):
                ...
                # Transfers (Tier-1 -> warehouse), brand-aware if provided
                for rec in (res.get("transfer_flows") or []):
                    rec2 = dict(rec)
                    rec2.setdefault("brand", "")
                    rec2.setdefault("center_idx", None)
                    lanes.append(rec2)

                # Inbound (Supply -> Tier-1 or -> warehouse), brand-aware if provided
