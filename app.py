...
from visualization import plot_network, summary, plot_flows
...
for i,tab in enumerate(tabs[:-1]):
    ...
    with tab:
        ...
        if st.session_state.get("run_target")==name:
            ...
            # ── Flow/brand controls (always visible, above the map) ─────────────────────────────
            ...
            # Determine which flow types exist to set a sensible default, but always show all options
            have_out = not lanes_df[lanes_df["lane_type"]=="outbound"].empty if "lane_type" in lanes_df.columns else False
            have_trn = not lanes_df[lanes_df["lane_type"]=="transfer"].empty if "lane_type" in lanes_df.columns else False
            have_inb = not lanes_df[lanes_df["lane_type"]=="inbound"].empty if "lane_type" in lanes_df.columns else False
            default_flows = ([t for t,ok in [("outbound",have_out),("transfer",have_trn),("inbound",have_inb)] if ok] or ["outbound"])
            flow_sel = st.multiselect("Flows to display", options=["outbound","transfer","inbound"], default=default_flows, help="Choose which flows to draw on the map.")
            brand_sel = st.selectbox("Brand filter", options=["__ALL__"] + brands_available, index=0, help="Show flows for one brand or all")
            # ── Map render (uses the selections above) ───────────────────────────────────────────
